...
class MutexLock {
public:
    explicit MutexLock(pthread_mutex_t *mu) : mu_(mu) { pthread_mutex_lock(this->mu_); }
...
class RWLock {
public:
    explicit RWLock(pthread_rwlock_t *mu, bool is_rwlock) : mu_(mu)
    {
        if (is_rwlock) {
...
class RecordLock {
public:
    RecordLock(port::RecordMutex *mu, const std::string &key) : mu_(mu), key_(key) { mu_->Lock(key_); }
