...
void RecordMutex::Lock(const std::string &key)
{
    ...
    if (it != records_.end()) {
        // log_info ("tid=(%u) >Lock key=(%s) exist, map_size=%u",
        // pthread_self(), key.c_str(), records_.size());
        RefMutex *ref_mutex = it->second;
        ref_mutex->Ref();
        mutex_.Unlock();

        ref_mutex->Lock();
        // log_info ("tid=(%u) <Lock key=(%s) exist", pthread_self(),
        // key.c_str());
    }
    else {
        // log_info ("tid=(%u) >Lock key=(%s) new, map_size=%u ++",
        // pthread_self(), key.c_str(), records_.size());
        ...
        ref_mutex->Lock();
        // log_info ("tid=(%u) <Lock key=(%s) new", pthread_self(),
        // key.c_str());
    }
}
...
void RecordMutex::Unlock(const std::string &key)
{
    ...
    // log_info ("tid=(%u) >Unlock key=(%s) new, map_size=%u --",
    // pthread_self(), key.c_str(), records_.size());
    if (it != records_.end()) {
        RefMutex *ref_mutex = it->second;
